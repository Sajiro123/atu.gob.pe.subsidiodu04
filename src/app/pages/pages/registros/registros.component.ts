...
import { RegistroVehicular, Usuario } from '../../../core/models/models';
...
export class RegistrosComponent implements OnInit {
  ...
  ngOnInit(): void {
    const user = this.resolveSession();
    if (user) {
  ...
  confirmarEnvio(): void {
    ...
    const envio = this.cargaService.confirmarEnvio(
      this.entidad,
      nOficio,
      sessionDoc,
    );
  ...
  badgeEstado(e: string): { label: string; css: string } {
    ...
    if (u === 'VIGENTE' || u === 'HABILITADO')
      return { label: e, css: 'badge-green' };
    if (u === 'SUSPENDIDO') return { label: e, css: 'badge-yellow' };
    if (u.includes('CANCELADO') || u.includes('INHABILITADO'))
      return { label: e, css: 'badge-red' };
